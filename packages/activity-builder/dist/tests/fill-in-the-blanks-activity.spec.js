...
describe('FillInTheBlanksActivity', () => {
    ...
    it('should be able to return wrong blanks', () => {
        ...
        activity.setOptions([new text_block_1.TextBlock('Option 1'), new text_block_1.TextBlock('Option 2')]);
        activity.setAnswer({ $$blank1$$: activity.getOptions()[0].id });
        expect(activity.checkAnswer({ $$blank1$$: activity.getOptions()[1].id })).toEqual(expect.objectContaining({ wrongBlanks: ['$$blank1$$'] }));
    });
    it('should return empty array if correct', () => {
        ...
        activity.setOptions([new text_block_1.TextBlock('Option 1'), new text_block_1.TextBlock('Option 2')]);
        activity.setAnswer({ $$blank1$$: activity.getOptions()[0].id });
        expect(activity.checkAnswer({ $$blank1$$: activity.getOptions()[0].id })).toEqual(expect.objectContaining({ wrongBlanks: [] }));
    });
});
